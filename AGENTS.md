...
Argus is a Rust 2021 workspace. `crates/argus-client/` contains the `argus` terminal UI, `crates/argusd/` contains the daemon plus `argus-hook`, and `crates/argus-protocol/` owns shared messages, framing, IDs, and transport types. Keep behavior near its owning crate and put cross-process contracts in `argus-protocol` — the pane API’s URL grammar lives there for exactly that reason. The daemon’s `state.rs` owns the tree; `state/panes.rs`, `state/sync.rs`, `state/git_ops.rs`, `state/hook.rs`, `state/session.rs`, and `state/tree.rs` carry panes, disk reconciliation, Git writes, the loopback receiver, session restore, and the tree lookup helpers. `store.rs` owns `runtime.db`, the SQLite store for everything Argus persists; `projects.toml` is read-only configuration. The client’s `app.rs` owns the model, with `app/nav.rs`, `app/input.rs`, `app/server.rs`, `app/pickers.rs`, `app/actions.rs`, and `app/mouse.rs` carrying the rest of `impl App`. Unit tests generally live beside implementation code in `#[cfg(test)] mod tests` blocks. `scripts/dev.ps1` supports isolated local development; `target/` is generated output. Consult `DESIGN.md` for current behavior and `TARGET.md`/`ROADMAP.md` for intended direction.
