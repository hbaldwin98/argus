...
//! and has to: the daemon shares no console with the client, so anything it
//! wrote would either land in the middle of the TUI or go to a console
//! window of its own. That left every line the daemon logged going nowhere,
//! and a daemon-side problem could only be looked into by abandoning the
//! client and starting `argusd` by hand — which is exactly when the
//! interesting state is gone.
//!
//! So it writes to a file as well. Stderr is kept because a daemon started
//! from a terminal for debugging should still print, and there it is the
...
/// The run before this one. Kept because a daemon that is starting has
/// often just stopped in a way somebody wants to read about, and truncating
/// on startup would destroy that log at exactly the wrong moment.
fn previous_log_path() -> PathBuf {
    argus_protocol::config_dir().join("argusd.log.1")
}

/// Opens this run's log, moving the last one aside.
///
/// `None` if the file cannot be opened. Logging to a file is a convenience
/// and must never be the reason a daemon fails to start, so every step here
/// is best-effort: an unwritable directory, or a previous log some other
/// process still holds open, costs the log and nothing else.
